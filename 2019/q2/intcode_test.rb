require 'minitest/autorun'
require 'minitest/spec'
require 'bigdecimal'

INPUT = [1,0,0,3,1,1,2,3,1,3,4,3,1,5,0,3,2,13,1,19,1,19,10,23,1,23,6,27,1,6,27,31,1,13,31,35,1,13,35,39,1,39,13,43,2,43,9,47,2,6,47,51,1,51,9,55,1,55,9,59,1,59,6,63,1,9,63,67,2,67,10,71,2,71,13,75,1,10,75,79,2,10,79,83,1,83,6,87,2,87,10,91,1,91,6,95,1,95,13,99,1,99,13,103,2,103,9,107,2,107,10,111,1,5,111,115,2,115,9,119,1,5,119,123,1,123,9,127,1,127,2,131,1,5,131,0,99,2,0,14,0].freeze

def opcode(code)
  pos = 0
  loop do
    case code[pos]
    when 1
      code[code[pos + 3]] = code[code[pos + 1]] + code[code[pos + 2]]
    when 2
      code[code[pos + 3]] = code[code[pos + 1]] * code[code[pos + 2]]
    when 99
      break
    else
      raise 'bad opcode'
    end
    pos += 4
  end
  code
end

describe 'intcode' do
  it '1,0,0,0,99' do
    assert_equal [2,0,0,0,99], opcode([1,0,0,0,99])
  end

  it '2,3,0,3,99' do
    assert_equal [2,3,0,6,99], opcode([2,3,0,3,99])
  end

  it '2,4,4,5,99,0' do
    assert_equal [2,4,4,5,99,9801], opcode([2,4,4,5,99,0])
  end

  it '1,1,1,4,99,5,6,0,99' do
    assert_equal [30,1,1,4,2,5,6,0,99], opcode([1,1,1,4,99,5,6,0,99])
  end

  it 'part 1' do
    input = INPUT.dup
    input[1] = 12
    input[2] = 2
    output = opcode(input)
    assert_equal 5866714, output[0]
  end

  it 'part 2' do
    noun = verb = -1
    (0..99).each do |a|
      (0..99).each do |b|
        input = INPUT.dup
        input[1] = a
        input[2] = b
        if opcode(input)[0] == 19690720
          # puts "100 * #{a} + #{b} == #{(100 * a) + b}"
          noun = a
          verb = b
          break
        end
      end
    end
    assert_equal 52, noun
    assert_equal 8, verb
    assert_equal 5208, ((100 * noun) + verb)
  end
end

